% This script tests the MCMC sampler (Gibbs & M3 & AE move)
%
% Version 1.0
% 15-March-2020
...
x=ave_adj{2};   % single adjacency matrix
...
tic
for i=1:N_sim
 [Z_chain,prob_chain,accep_r_array]=MCMC_allocation_AE(x,K,priorpra);
...
figure
imagesc(group_adj{1,1}{1,3}(1:16,1:16))
...
%colormap(jet);
%colorbar;
set(gca,'position',[0.1,0.2,0.6,0.7] );
set(gca,'fontsize',18)
set(gcf,'unit','normalized','position',[0.1,0.2,0.18,0.27]);
%--------------------------------------------------------------------------
% plot Markov chain
figure
imagesc(Z_chain)
