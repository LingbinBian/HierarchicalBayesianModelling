function [] = infer_real(subid,session_n,K_seg,W,S,thres)
...
[AdjaCell,netpara]=adjacencyCellArray(Timeseries,W,K_seg,thres);

% Discrepancy index
[discindex,para_infer]=discrepindex(AdjaCell,netpara,S,thres);
...
if session_n==1
    Inference_LR=fullfile(Inference_path,'Global_fitting_thres_real',['infer_tfMRI','_','K',num2str(K_seg),'_','W',num2str(2*W),],subid,'inference_WM_LR/infer_LR');
    save(Inference_LR,'cumenergy','K_seg','W');
    
elseif session_n==2
    Inference_RL=fullfile(Inference_path,'Global_fitting__thres_real',['infer_tfMRI','_','K',num2str(K_seg),'_','W',num2str(2*W),],subid,'inference_WM_RL/infer_RL');
